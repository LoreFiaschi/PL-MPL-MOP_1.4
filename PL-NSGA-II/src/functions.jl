function _nsga(::indiv{G,Ph,Y,C}, sense, popSize, nbGen, init, z, fdecode, fdecode!,
    ...
    PL_fast_non_dominated_sort!(view(P, 1:popSize), sense, lowestpower)

    @showprogress refreshtime for gen = 1:nbGen

        for i = 1:2:popSize

            pa = tournament_selection(P, popSize)
            pb = tournament_selection(P, popSize)
		...
		PL_fast_non_dominated_sort!(P, sense, lowestpower)
...
function fast_non_dominated_sort!(pop::AbstractVector{T}, sense, gp::Int, firstrank::Int) where {T}
    n = length(pop)
    ...
    @inbounds for i in 1:n
        for j in i+1:n
            if dominates(sense, pop[i], pop[j], gp)
                push!(pop[i].dom_list, j)
                pop[j].dom_count += 1
            elseif dominates(sense, pop[j], pop[i], gp)
...
function PL_fast_non_dominated_sort!(pop::AbstractVector{T}, sense, minpower::Int) where {T}
    n = length(pop)
    ...
    for gp in 1:minpower
        ...
        for i in 1:glob_highest_rank
            ...
            last_rank_used = fast_non_dominated_sort!(view(pop, ind:nextind-1), sense, gp, last_rank_used+1)
            ind = nextind
        end
