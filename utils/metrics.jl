...
function preprocess(pop_array::Vector{Vector{Matrix{T}}}, level_size::Vector{S}) where {T<:Real,S<:Integer}
	...
	for i in eachindex(pop_array)
        ...
        newpop_array[i] = Vector{Individual}(pop_size)
        for j=1:pop_size
...
function compute_GD_performance(pop::Vector{Individual}, true_pareto::Vector{Individual}, minimize, ishibuchi::Bool=false)
    performance = 0
    for ind in pop
        if ind.dominated
            min_dist = Inf
            for true_ind in true_pareto
                dist = compute_distance(true_ind.cost, ind.cost, minimize, ishibuchi)
                if dist < min_dist
                    min_dist = dist
                    if min_dist == 0
                        break
...
function compute_IGD_performance(pop::Vector{Individual}, true_pareto::Vector{Individual}, pop_idx::T, minimize, ishibuchi::Bool=false) where{T<:Integer}
    performance =  0
    for true_ind in true_pareto
...
function _GD_IGD_performances(pop_array::Vector{Vector{Matrix{T}}}, level_size::Vector{S}, minimize, separated::Bool=false, ishibuchi::Bool=false) where {T<:Real,S<:Integer}
    pop_array, whole_pop = preprocess!(pop_array, level_size)
    determine_domination!(whole_pop, level_size, minimize) #Notice: domination information is added also in pop_array
	...
	if separated
		...
		for i in eachindex(pop_array)
			performance_vector_GD[i] = compute_GD_performance(pop_array[i], true_pareto, minimize, ishibuchi)
			performance_vector_IGD[i] = compute_IGD_performance(pop_array[i], true_pareto, i, minimize, ishibuchi)
	...
	for i in eachindex(pop_array)
	    performance_vector[i] = compute_GD_performance(pop_array[i], true_pareto, minimize, ishibuchi)
	    igd_performance = compute_IGD_performance(pop_array[i], true_pareto, i, minimize, ishibuchi)
	    performance_vector[i] = (igd_performance > performance_vector[i])? igd_performance : performance_vector[i]
	end
	return performance_vector
end

GD_IGD_performances(pop_array::Vector{Vector{Matrix{T}}}, level_size::Vector{S}, minimize, separated::Bool=false, ishibuchi::Bool=false) where {T<:Real,S<:Integer} = _GD_IGD_performances(pop_array, level_size, minimize, separated, ishibuchi)
end #module
