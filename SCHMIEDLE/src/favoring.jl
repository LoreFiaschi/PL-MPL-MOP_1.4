...
function determine_favoring!(population, priorities::Vector{T}, num_priorities::T) where T<:Integer
    ...
    for i=1:pop_size
        for j=(i+1):pop_size
            favoring = favored(population[i], population[j], priorities, num_priorities)
    ...
    MG = ComputeMetaGraph(SCC, numComponents, population, priorities, num_priorities)
...
function favored(ind1, ind2, priorities::Vector{T}, num_priorities::T) where T<:Integer
...
function ComputeMetaGraph(SCC, numComponents::T, population, priorities::Vector{T}, num_priorities::T) where T<:Integer
    
    G = SimpleDiGraph(numComponents,0)
    
    for i=1:numComponents
        for j=i+1:numComponents
            favoring = favored(population[SCC[i][1]], population[SCC[j][1]], priorities, num_priorities)
            favoring > 0 && add_edge!(G, i, j)
            favoring < 0 && add_edge!(G, j, i)
        end
    end
